...
def error_channel_flow_FE_unsteady_inlet (steps = 3,return_stability=False, name='', guess=None, project=[],alpha=0.99):
    ...
    while count < tend:
        ...
        #     divu[1:-1,1] = divu[1:-1,1] #+ u_bc_left_wall(t)(0)/dx
        #     ub = np.ones_like(divu[1:-1,1])*u_bc_left_wall(t)(0)
        #     plt.imshow(divu[1:-1,1:-1], origin='bottom')
        #     plt.colorbar()
        #     plt.show()
        #     plt.plot(divu[1:-1,1],label='divu')
        #     plt.plot(ub,label='r(t)')
        #     # plt.ylim([0,2])
        #     # plt.colorbar()
