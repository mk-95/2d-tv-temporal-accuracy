...
def error_RK3_with_post_projection (steps = 3,return_stability=False, name='regular',guess=None,project=[1,1],alpha=0.999,post_projection=False):
    # problem description
    ...
    while count < tend:
        ...
        if count >2:
            ...
            d1=0
            d2,d3 = project
            f1x, f1y, f2x, f2y = f.Guess([pn, pnm1,pnm2], order=guess, integ='RK3',type=name)

        elif count <= 2: # compute pressures for 2 time steps
            d1=1
            d2 = 1
            d3 = 1
            f1x,f1y,f2x,f2y =  f.Guess([pn,pnm1,pnm2],order=None,integ='RK3',type=name)
