...
def error_lid_driven_cavity_RK3_unsteady_bcs (steps = 3,return_stability=False, name='heun', guess=None, project=[],alpha=0.99):
    ...
    u_bc_top_wall = lambda t: lambda yv: np.cos(at(t))
    u_bc_bottom_wall = lambda xv: 0
    u_bc_right_wall =  lambda yv: 0
    u_bc_left_wall = lambda xv: 0

    v_bc_top_wall = lambda t: lambda yv: np.sin(at(t))
    v_bc_bottom_wall = lambda xv: 0
    v_bc_right_wall = lambda yv: 0
    v_bc_left_wall = lambda xv: 0
    ...
    while count < tend:
        ...
        f.right_wall(uh3, vh3, u_bc_right_wall, v_bc_right_wall)
...
# error_lid_driven_cavity_RK3_unsteady_bcs (steps = 2000,return_stability=False, name='heun', guess=None, project=[1,1],alpha=0.99)